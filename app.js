...
let spotX=0,spotY=0,spotFrame=0;
...
addEventListener('mousemove',e=>{
  ...
  if(!spotFrame)spotFrame=requestAnimationFrame(paintSpotlight);
},{passive:true});
addEventListener('mouseleave',()=>spotlight.style.opacity=0);
...
function closeOverlay(){
  ...
  setTimeout(hidePanels,180);
}
document.querySelectorAll('.native-open').forEach(el=>el.addEventListener('click',()=>showNativePanel(el.dataset.panel)));
const ataturkLink=document.getElementById('ataturkLink');
ataturkLink.onclick=e=>{e.preventDefault();showNativePanel('ataturkPanel')};
